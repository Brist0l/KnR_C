...
int main(){
	// what appears to be "charecters" on the keyboards are stored as bit streams internally
	// so to store such "charecters" any integer data type can be used but char is prefered.
	int c; // over here int is used so that EOF works. getchar works in such a manner that when the input
	       ...
	       // char would only be able to hold one chararecter and not the EOF which would follow ahead.
